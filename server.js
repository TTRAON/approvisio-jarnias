...
app.get('/api/alobees/sites', auth, async (req, res) => {
  ...
  try {
    ...
    let filtered = all;
    if (q) filtered = all.filter(function (s) {
      var blob = [s.name, s.nom, s.reference, s.ref, s.customer, s.client, s.address, s.complement, s.city, s.zipCode, s.zip_code]
        .map(function (v) { return v == null ? '' : ('' + v); }).join(' ').toLowerCase();
...
app.post('/api/lists/:name/set', auth, adminOrDepot, async (req, res) => {
  try {
    const name = req.params.name;
    if (!LIST_NAMES.includes(name)) return res.status(400).json({ error: 'Liste inconnue' });
    const data = Array.isArray(req.body && req.body.data) ? req.body.data : [];
