...
// Renvoie false si l'admin a désactivé les emails (CONFIG.mailNotifs === false).
// Par défaut (absent ou true), les emails sont autorisés.
async function mailAllowed() {
  try {
    const { rows } = await pool.query("SELECT data FROM app_lists WHERE name = 'config'");
    if (rows[0] && rows[0].data) {
      const d = rows[0].data;
      const cfg = Array.isArray(d) ? (d[0] || {}) : d;
      if (cfg && cfg.mailNotifs === false) return false;
    }
  } catch (e) { /* en cas de doute on n'empêche pas l'envoi */ }
  return true;
}

async function sendEmail(to, subject, html) {
  if (!RESEND_API_KEY || !to) return; // pas de clé ou pas d'email : on saute silencieusement
  if (!(await mailAllowed())) return; // emails coupés par l'admin
  try {
